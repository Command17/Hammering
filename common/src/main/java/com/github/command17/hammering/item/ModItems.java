...
public final class ModItems {
    ...
    public static final RegistrySupplier<Item> NETHERITE_HAMMER = registerHammer("netherite_hammer", Tiers.NETHERITE, 6, -2.75f, Hammering.CONFIG.netheriteHammerExtraDurabilityMultiplier.get(), of().fireResistant());

    private static RegistrySupplier<Item> registerHammer(String name, Tier tier, int attackDamage, float attackSpeed, int extraDurability, Item.Properties properties) {
        return register(name,
                () -> new HammerItem(
                        tier,
                        properties.attributes(DiggerItem.createAttributes(tier, attackDamage, attackSpeed)),
                        Hammering.CONFIG.hammerDurabilityMultiplier.get() + extraDurability
                ));
    }
    ...
    private static RegistrySupplier<Item> registerHammer(String name, Tier tier, int attackDamage, float attackSpeed, Item.Properties properties) {
        return registerHammer(name, tier, attackDamage, attackSpeed, 0, properties);
    }
